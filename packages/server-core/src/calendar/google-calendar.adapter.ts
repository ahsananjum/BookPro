...
export class GoogleCalendarAdapter implements CalendarProvider {
    ...
    async createEvent(accessToken: string, calendarId: string, event: GoogleEventPayload): Promise<any> {
        ...
        const sendUpdatesParam = event.attendees && event.attendees.length > 0 ? "?sendUpdates=all" : "";
        const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodedCalId}/events${sendUpdatesParam}`, {
            method: "POST",
            headers: {
    ...
    async updateEvent(accessToken: string, calendarId: string, eventId: string, event: GoogleEventPayload): Promise<any> {
        ...
        const sendUpdatesParam = event.attendees && event.attendees.length > 0 ? "?sendUpdates=all" : "";
        const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodedCalId}/events/${encodedEventId}${sendUpdatesParam}`, {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${accessToken}`,
                "Content-Type": "application/json",
    ...
    async deleteEvent(accessToken: string, calendarId: string, eventId: string): Promise<void> {
        const encodedCalId = encodeURIComponent(calendarId);
        const encodedEventId = encodeURIComponent(eventId);
        const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/${encodedCalId}/events/${encodedEventId}?sendUpdates=all`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${accessToken}` },
        });
