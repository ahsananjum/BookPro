...
describe("Strict Bidirectional AI Role Isolation Matrix", () => {
  ...
  describe("Staff Boundary Enforcement (Deny Consumer Checkout Tools)", () => {
    ...
    it("should reject staff calling autoRescheduleAppointment with CROSS_ROLE_TOOL_DENIED", async () => {
      await expect(
        registry.execute("conv-2", "autoRescheduleAppointment", {}, staffCtx, "idemp-auto-resched")
      ).rejects.toThrow(ForbiddenException);

      try {
        await registry.execute("conv-2", "autoRescheduleAppointment", {}, staffCtx, "idemp-auto-resched");
      } catch (err: any) {
        expect((err.getResponse() as any)?.code).toBe("CROSS_ROLE_TOOL_DENIED");
      }
    });
