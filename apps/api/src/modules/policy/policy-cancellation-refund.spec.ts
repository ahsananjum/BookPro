...
describe('P0-07: Cancellation Quotes & Refund Accounting (Unit & Integration)', () => {
    ...
    beforeEach(async () => {
        ...
        mockPrisma = {
            ...
            refundRecord: {
                findFirst: jest.fn().mockImplementation(async (q) => {
                    return inMemoryDb.refundRecords.find(
                        (r) =>
                            (!q.where.id || r.id === q.where.id) &&
                            (!q.where.organizationId || r.organizationId === q.where.organizationId) &&
                            (!q.where.idempotencyKey || r.idempotencyKey === q.where.idempotencyKey) &&
                            (!q.where.status || r.status === q.where.status)
                    ) || null;
                }),
                create: jest.fn().mockImplementation(async (q) => {
            ...
            providers: [
                ...
                { provide: ScheduleGuardService, useValue: { acquireSlotLocks: jest.fn().mockResolvedValue({ release: jest.fn() }) } },
                {
                    ...
                    useValue: {
                        ...
                        lockAndExecute: jest.fn().mockImplementation((k, f) => f()),
                        executeIdempotent: jest.fn().mockImplementation(async (opts, fn) => ({ statusCode: 200, data: await fn(), fromCache: false })),
                ...
                { provide: 'RealtimeService', useValue: { broadcastEvent: jest.fn().mockResolvedValue(undefined) } },
                { provide: 'RedisService', useValue: { delPrefix: jest.fn().mockResolvedValue(0) } },
            ],
        }).compile();
    ...
    describe('4. Deposit & Partial Upfront Payment Bounds', () => {
        ...
        it('should deduct $25 fixed fee from $30 deposit and refund remaining $5', async () => {
            const appt = await mockPrisma.appointment.create({
                data: {
                    ...
                    createdAt: new Date(Date.now() - 48 * 3600 * 1000), // Booked 48h ago (past grace period)
                },
            });
        ...
        it('should cap penalty fee at captured deposit amount when fee exceeds deposit', async () => {
            ...
            const appt = await mockPrisma.appointment.create({
                data: {
                    ...
                    endAt: new Date(Date.now() + 3 * 3600 * 1000),
                    createdAt: new Date(Date.now() - 48 * 3600 * 1000), // Booked 48h ago
                },
            });
        ...
        it('should grant 100% full refund and $0 fee when appointment was just booked within cooling-off grace period', async () => {
            inMemoryDb.policyConfigs[0].cancelFeeValue = 2500; // $25 fixed fee inside 24h cutoff
            ...
            const appt = await mockPrisma.appointment.create({
                data: {
                    ...
                    status: 'CONFIRMED',
                    paymentStatus: 'PARTIALLY_PAID',
                    startAt: new Date(Date.now() + 2 * 3600 * 1000), // Only 2h away
                    endAt: new Date(Date.now() + 3 * 3600 * 1000),
                    createdAt: new Date(Date.now() - 30 * 60 * 1000), // Booked 30 minutes ago!
                },
            });
            ...
            await mockPrisma.paymentRecord.create({
                data: {
                    organizationId: orgId,
                    appointmentId: appt.id,
                    providerPaymentId: 'pi_deposit_grace',
                    amountCents: 3000,
                    currency: 'USD',
                    status: PaymentRecordStatus.SUCCEEDED,
                    idempotencyKey: 'ik_dep_grace',
                },
            });

            const quote = await policyService.calculateCancellationQuote(orgId, appt.id);
            expect(quote.capturedBalanceCents).toBe(3000);
            expect(quote.feeCents).toBe(0); // 100% fee waived due to grace period!
            expect(quote.refundableCents).toBe(3000); // Full refund!
            expect(quote.reason).toContain('grace period');
        });

        it('should correctly evaluate multi-currency captured balance in appointment currency', async () => {
            inMemoryDb.policyConfigs[0].cancelCutoffHours = 24;
            inMemoryDb.policyConfigs[0].cancelFeeType = 'PERCENTAGE';
            inMemoryDb.policyConfigs[0].cancelFeeValue = 50; // 50% fee
            ...
            const appt = await mockPrisma.appointment.create({
                data: {
                    ...
                    locationId,
                    serviceId,
                    priceCents: 150000, // 1500 PKR
                    currency: 'PKR',
                    status: 'CONFIRMED',
                    paymentStatus: 'PARTIALLY_PAID',
                    startAt: new Date(Date.now() + 2 * 3600 * 1000),
                    endAt: new Date(Date.now() + 3 * 3600 * 1000),
                    createdAt: new Date(Date.now() - 48 * 3600 * 1000), // Past grace period
                },
            });
            ...
            await mockPrisma.paymentRecord.create({
                data: {
                    organizationId: orgId,
                    appointmentId: appt.id,
                    providerPaymentId: 'pi_deposit_multi',
                    amountCents: 157, // $1.57 USD gateway charge
                    currency: 'USD',
                    status: PaymentRecordStatus.SUCCEEDED,
                    idempotencyKey: 'ik_dep_multi',
                    metadata: {
                        originalAmountCents: 43500, // 435 PKR deposit
            ...
            expect(quote.capturedBalanceCents).toBe(43500); // Correct 435 PKR!
            expect(quote.feeCents).toBe(21750); // 50% fee in PKR!
            expect(quote.refundableCents).toBe(21750); // 50% refund in PKR!
        });
    });
        ...
        it('should create ReconciliationIncident and flag REQUIRES_RECONCILIATION if provider succeeds but local DB transaction fails', async () => {
            ...
            await expect(
                refundsService.processRefund(orgId, {
                    ...
                    paymentRecordId: pay.id,
                    amountCents: 5000,
                    idempotencyKey: 'ik_refund_local_fail',
                })
    ...
    describe('12. Authoritative Server Time & Tampering Defense', () => {
        it('should mark quote as disallowed if appointment start time is already in the past', async () => {
            inMemoryDb.policyConfigs.push({
                id: 'pol-time-tamper',
                organizationId: orgId,
                serviceId,
            ...
            const pastAppt = await mockPrisma.appointment.create({
                data: {
                    organizationId: orgId,
                    locationId,
                    serviceId,
                    ...
                    status: 'CONFIRMED',
                    paymentStatus: 'PAID',
                    startAt: new Date(Date.now() - 3600 * 1000), // 1 hour in the past!
                    endAt: new Date(Date.now() - 1800 * 1000),
                    createdAt: new Date(Date.now() - 48 * 3600 * 1000),
                },
            });

            const quote = await policyService.calculateCancellationQuote(orgId, pastAppt.id);
            expect(quote.isAllowed).toBe(false);
            expect(quote.reason).toContain('already passed');

            await expect(
                policyService.validateCancellationQuote(orgId, pastAppt.id, quote.quoteVersion)
            ).rejects.toThrow('already passed');
        });

        it('should reject cancellation quote validation if appointment is already CANCELLED', async () => {
            const cancelledAppt = await mockPrisma.appointment.create({
                data: {
                    organizationId: orgId,
                    locationId,
                    serviceId,
            ...
            await expect(
                policyService.validateCancellationQuote(orgId, cancelledAppt.id)
